...
def parse_args():
    parser = argparse.ArgumentParser(description="Get mutations from a sample path")
    ...
    parser.add_argument(
        "-r", "--reference", type=str, required=True, help="Path to reference sequence"
    )
...
def compare_seqs(ref, root):
    ...
    for i, nuc in enumerate(
        ...
        if (
            ...
            additonal_muts[i + 1] = {"ref": ref_nuc.upper(), "root": root_nuc.upper()}
...
def main():
    ...
    if sample_muts_df[sample_muts_df["sample"] == ref.id].shape[0] > 0:
        print("Reference {} is present in sample mutations file.".format(ref.id))
    ...
    # else generate the root sequence
        ...
        print(f"Reference {ref.id} not present in sample mutations file.")
        ...
        for sample_id, muts_str in tqdm(
            valid.itertuples(index=False, name=None),
            desc="Generating root sequences",
            total=len(valid),
        ):
            # build root mutations and fetch the sequence by direct dict lookup
            root_muts = reverse_muts_to_root(get_muts({"mutations": muts_str}))
            seq = seqs.get(sample_id)
            if seq is None:
                raise ValueError(f"Sample {sample_id} not found in FASTA file.")
    ...
    lineage_paths = parse_tree_paths(
        pd.read_csv(args.lineage_paths, sep="\t").fillna("")
    )

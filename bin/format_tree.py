...
def parser():
    ...
    parser.add_argument(
        "-f",
        "--format",
        help="Format of the nexus file",
        choices=["nexus", "newick"],
        default="nexus",
    )
    parser.add_argument(
        "-r", "--reformat", help="Reformat using ete3", action="store_true"
    )
...
def convert():
    ...
    tree.write_to_path(args.output, schema="newick")

    if args.reformat:
        handle = open(args.output, 'r')
        for line in handle:
            l=line.strip('\n')
            if '(' in l:
                treeString_start=l.index('(')
        treeString=l[treeString_start:]
